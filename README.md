...
The package contains both the text of every address, and meta data that includes the name of the president who delivered the address, the date of delivery, and the president's party affiliation. The following code creates a dataframe that binds these two objects together, since the textnets package currently requires text to be inside a dataframe. Or specifically, the input of textnets is a dataframe where each row represents a document and there are at least two columns: one containing the text of the document, and another providing the metadata used to generate the network. At the very least, the latter should be a unique id for the document.
...
The textnet package includes two functions to prepare texts for analysis. The `prep_text` function creates text networks based upon all types of words, and the `prep_text_noun_phrases` prepares text networks based upon nouns and noun phrases that appear in a given document. Users may prefer to create networks based on nouns or noun phrases because previous studies have shown that such parts of speech are more useful for mapping the topical content of a text than other parts of speech such as verbs or adjectives (e.g. Rule, Cointet, and Bearman 2015).
